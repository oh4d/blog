# Livinig On The Edge - How to debug a PHP app inside kubernetes pod?

Containers where designed to isolate an application, so it can get a consistent environemnt and you can run it anywhere and expect same results.
In practice, things are a bit different.... and it is common to deal with an app that works perfectly on dev and staging environtments, but looses it on production. On that moments, the developers are so sorry they didn't put enough logs on their little app, but its too late. That brings us back to the old days, and have know choice but to debug on production.
...
- "6? comeon, thats to many steps. nobodys gonna read this through"
- "chill, theyre baby-small steps. After a few times practice I could finish the whole process in less than a minute. Besides, you have no choice, cause you got no logs and your production is down."

### How its going to work (skip this part if prod is really down);

Lets see how PHP debugging actually work. well, once you enable debugging on your text edtior, its becoming a server - starts  listening on the debugging port, usually 9000. On the other hand, your app, which runs with xdebug extension enabled, is the client - it is logging debugging data right to that port 9000.
What I do is to use ngrok to publicly expose my local port 9000, and once it is exposed - install xdebug on the app container and send debugging logs stright out to my exposed port.

### Your side configurations:

1. install ngrok:
   
Instructions [here](https://ngrok.com/download). Its free.
...
install & run xdebug in the container:

```sh
# apk if you run alpine linux. Otherwise use your own package manager:
apk add xdebug 
